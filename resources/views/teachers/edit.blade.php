...
@section('header-title', 'Edit Teacher')
@section('header-subtitle', 'Modify teacher information')
...
@section('content')
    <div class="p-6">
        <form method="POST" action="{{ route('teachers.update', $teacher->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="address_id" 
                        ...
                        @foreach($addresses as $address)
                            <option value="{{ $address->id }}" {{ old('address_id', $teacher->address_id) == $address->id ? 'selected' : '' }}>
